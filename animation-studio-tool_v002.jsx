...
function downloadPDF(project, schedule, quote, currentUser) {
  ...
  <style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    th { background:#1a1a2e; color:#fff; padding:8px 10px; text-align:left; font-weight:700; font-size:11px; text-transform:uppercase; letter-spacing:.05em; }
...
function AuthScreen({ onLogin }) {
  ...
  const iStyle = { width: "100%", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 10, padding: "12px 16px", color: "#F9FAFB", fontSize: 14, fontFamily: "inherit", outline: "none", boxSizing: "border-box" };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d1a", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Noto Sans KR', sans-serif" }}>
      <div style={{ position: "fixed", inset: 0, background: "radial-gradient(ellipse 60% 50% at 50% 0%, rgba(200,16,46,0.15) 0%, transparent 70%)", pointerEvents: "none" }} />
      <div style={{ position: "relative", width: 380, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 20, padding: "40px 36px", backdropFilter: "blur(12px)" }}>
        {/* 타이틀 */}
        <div style={{ marginBottom: 32 }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 900, color: "#F9FAFB", letterSpacing: "-0.02em" }}>Schedule & <span style={{ color: "#C8102E" }}>Quote Tool</span></div>
        </div>

        {mode === "pending" ? (
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 40, marginBottom: 16 }}>⏳</div>
            <h2 style={{ color: "#F9FAFB", fontSize: 18, fontWeight: 800, marginBottom: 8 }}>가입 신청 완료</h2>
            <p style={{ color: "#9CA3AF", fontSize: 13, lineHeight: 1.6 }}>관리자 승인 후 로그인 가능합니다.<br />관리자에게 승인을 요청하세요.</p>
            <button onClick={() => setMode("login")} style={{ ...btnStyle, marginTop: 24, background: "rgba(255,255,255,0.08)" }}>로그인으로 돌아가기</button>
          </div>
        ) : (
          <>
            <h2 style={{ color: "#F9FAFB", fontSize: 20, fontWeight: 900, marginBottom: 24 }}>{mode === "login" ? "로그인" : "회원가입"}</h2>
            ...
            {mode === "login" && <div style={{ marginTop: 16, padding: "10px 12px", background: "rgba(255,255,255,0.03)", borderRadius: 8, fontSize: 11, color: "#6B7280" }}>
...
function AdminPanel({ currentUser, onClose }) {
  ...
  const rowStyle = { display: "grid", gridTemplateColumns: "1fr 100px 120px auto", gap: 10, alignItems: "center", padding: "10px 14px", borderRadius: 10, background: "rgba(255,255,255,0.03)", marginBottom: 8 };
  const iStyle = { width: "100%", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 8, padding: "9px 12px", color: "#F9FAFB", fontSize: 13, fontFamily: "inherit", outline: "none", boxSizing: "border-box" };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)" }}>
      <div style={{ width: 640, maxHeight: "80vh", background: "#13131f", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 20, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "20px 24px", borderBottom: "1px solid rgba(255,255,255,0.08)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h2 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: "#F9FAFB" }}>👑 Admin Panel</h2>
          <button onClick={onClose} style={{ background: "none", border: "none", color: "#9CA3AF", fontSize: 20, cursor: "pointer" }}>×</button>
        </div>
        <div style={{ display: "flex", gap: 4, padding: "12px 24px 0", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: 24 }}>
          {tab === "pending" && (
            ...
            pending.map(u => (
              <div key={u.id} style={rowStyle}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 13, color: "#F9FAFB" }}>{u.email}</div>
                  <div style={{ fontSize: 11, color: "#6B7280" }}>{new Date(u.createdAt).toLocaleString("ko-KR")}</div>
                </div>
                <select value="member" onChange={e => {}} style={{ background: "#1a1a2e", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 6, padding: "5px 8px", color: "#F9FAFB", fontSize: 12, fontFamily: "inherit" }}>
              ...
              <div key={u.id} style={rowStyle}>
                ...
                {u.id !== "admin" ? (
                  <select value={u.role} onChange={e => changeRole(u.id, e.target.value)}
                    style={{ background: "#1a1a2e", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 6, padding: "5px 8px", color: "#F9FAFB", fontSize: 12, fontFamily: "inherit" }}>
                    <option value="member">Member</option>
                    <option value="senior">Senior</option>
          ...
          {tab === "pw" && (
            <div style={{ maxWidth: 380, display: "flex", flexDirection: "column", gap: 12 }}>
              <p style={{ color: "#9CA3AF", fontSize: 12, marginBottom: 4 }}>계정의 비밀번호를 변경합니다. 현재 비밀번호 확인 후 변경됩니다.</p>
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d1a", color: "#F9FAFB", fontFamily: "'Noto Sans KR', 'IBM Plex Sans KR', sans-serif" }}>
      {showAdmin && <AdminPanel currentUser={currentUser} onClose={() => setShowAdmin(false)} />}

      {/* 배경 효과 */}
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, background: "radial-gradient(ellipse 70% 40% at 50% -10%, rgba(200,16,46,0.1) 0%, transparent 70%)" }} />
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 960, margin: "0 auto", padding: "28px 24px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div>
              ...
              <h1 style={{ margin: 0, fontSize: 22, fontWeight: 900, letterSpacing: "-0.02em", lineHeight: 1.1 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end", alignItems: "center" }}>
            {/* 현재 유저 배지 */}
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 12px", background: "rgba(255,255,255,0.05)", borderRadius: 8, border: "1px solid rgba(255,255,255,0.1)" }}>
              ...
              <span style={{ fontSize: 12, color: "#9CA3AF" }}>{currentUser.email}</span>
            </div>
            ...
            <button onClick={() => setShowSaved(s => !s)} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, padding: "7px 14px", color: "#9CA3AF", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>
              📂 ({projects.length})
            </button>
            ...
            <button onClick={handleLogout} style={{ background: "none", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: "7px 12px", color: "#6B7280", fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>로그아웃</button>
          </div>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 24 }}>
          <button onClick={() => { const i = TABS.findIndex(t => t.id === activeTab); if (i > 0) setActiveTab(TABS[i-1].id); }}
            ...
            style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, padding: "10px 20px", color: "#9CA3AF", fontSize: 13, cursor: "pointer", fontFamily: "inherit", opacity: activeTab === TABS[0].id ? 0.3 : 1 }}>← 이전</button>
